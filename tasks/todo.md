...
- `hashFor()` omitted `issue=` for "All poles" while `init()` treated a bare hash as the Any-issue
  default, so the two disagreed: Back out of a record landed on a LARGER list, and Copy link from
  All poles handed the recipient Any issue. One `flagFor()` rule now serves first load and popstate,
  and every view writes itself, All poles included.
- An area with no condition issues would have opened on an empty page; the default degrades to
  All poles when the Any-issue population is zero.
- A malformed percent-escape in a root `#pole=` link threw inside the fetch `.then()` and rendered
  "No areas are published yet" over a site with five. Decode is guarded.
- The hero caption counted drawn markers, not findings: a pole with two attachments was reported as
  having one, because the third mark did not fit the render budget. It counts the record now.
- `data/osm/greenpoint-brooklyn-new-york/roads.json` could not be regenerated by any committed
  command (`roadcover.py` exits at the boundary lookup for a slug that geocodes to an OSM way), so
  on a fresh clone Greenpoint's street names would have vanished silently. `roadcover.py
  --roads-only` fetches the cache before the boundary step.
- `nearest_street()` now refuses a radius larger than the cell rather than silently missing
  segments outside the 3x3 neighbourhood, and `tests/test_roadcover.py` checks the index against a
  full scan over a lattice, across the meridian, the equator and southern latitudes.
- `st_m` was written to every bundle and read by nothing; it is now the provenance on the location
  line ("Nearest named road, about 7 m from the estimated position").
- Also: search no longer re-renders the open record on every keystroke; the "Best imagery" badge
  returns nothing on a tie and carries a title saying what it measures; About documents search.

### Known and not addressed
- Adding `st`/`st_m` changes `meta.version`, which orphans browser-local review decisions (the key
  is `polepass-review:<slug>:<version>`). Inherent to any record change; export/import is the path.
